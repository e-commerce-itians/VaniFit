...
export default function MessageDialog(
  ...
  observer(componentID, () => compLoaded(message, type, onClose));
    ...
    <div component="${componentID}" class="${componentID}" id="messageDialogOverlay">
      <div class="message-dialog-container ${type}-dialog">
        ...
        <div class="message-dialog-actions">
          <button id="confirmBtn" class="confirm-btn ${type}-btn">OK</button>
        </div>
      </div>
    </div>
  `;
...
const compLoaded = (message, type, onClose) => {
  ...
  function handleOverlayClick(event) {
    if (event.target === document.getElementById("messageDialogOverlay")) {
      handleClose();
    }
  }
